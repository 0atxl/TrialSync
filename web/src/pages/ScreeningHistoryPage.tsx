...
export function ScreeningHistoryPage() {
  ...
  const requestedState = searchParams.get('result')
  const [state, setState] = useState<'all' | ScreeningState>(
    requestedState && screeningStates.includes(requestedState as ScreeningState)
      ? requestedState as ScreeningState
      : 'all',
  )
  ...
  function changeState(nextState: 'all' | ScreeningState) {
    ...
    setSearchParams((current) => {
      ...
      if (nextState === 'all') next.delete('result')
      else next.set('result', nextState)
      return next
    })
  }
