...
export function ScreeningDetailPage() {
  ...
  return <section className="route-entry workspace-page">
    ...
    <header className="result-hero"><div><p className="eyebrow">Educational pre-screening result</p><h1 className={`result-title state-${screening.overall_state}`}>{stateLabel(screening.overall_state)}</h1><p><strong>{screening.patient_snapshot.display_name}</strong> · {screening.patient_snapshot.external_id}<br />{screeningTrialLabel(screening)}</p></div><div className="counts" aria-label="Criterion counts"><span><strong>{screening.counts.pass_count}</strong> pass</span><span><strong>{screening.counts.fail_count}</strong> fail</span><span><strong>{screening.counts.unknown_count}</strong> unknown</span></div></header>
    <div className="disclaimer">Synthetic-data educational prototype. This is not medical advice, enrollment guidance, or a clinical decision.</div>
    <section className="snapshot-panel"><div><p className="eyebrow">Immutable patient snapshot</p><h2>{screening.patient_snapshot.display_name}</h2><p>{screening.patient_snapshot.date_of_birth ? `Born ${screening.patient_snapshot.date_of_birth}` : 'Date of birth not recorded'}{screening.patient_snapshot.sex ? ` · ${screening.patient_snapshot.sex}` : ''}</p></div><div className="snapshot-facts">{screening.patient_snapshot.facts.length ? screening.patient_snapshot.facts.slice(0, 6).map((fact) => <span key={fact.id}><strong>{fact.concept}</strong>{fact.value_numeric ?? fact.value_text ?? fact.assertion} {fact.unit}</span>) : <span>No additional structured facts in this snapshot.</span>}</div></section>
    <section className="criteria-section"><div className="section-heading"><div><p className="eyebrow">Criterion evidence</p><h2>Unknown criteria appear first</h2></div></div>{ordered.map((item) => <article className={`evaluation evaluation-${item.result}`} id={`criterion-${item.id}`} key={item.id}><div className="evaluation-head"><span className="criterion-order">{item.criterion_order}</span><div><span className="record-kind">{item.criterion_kind}</span><h3>{item.criterion_source_text}</h3></div><span className={`state state-${item.result}`}>{item.result}</span></div><p className="canonical">{item.canonical_explanation}</p><div className="evidence-grid"><div><strong>Assessment</strong><p>{reasonLabel(item.reason_code)}</p></div><div><strong>Recorded evidence</strong>{item.evidence.length ? <ul>{item.evidence.map((evidence, index) => <li key={`${evidence.fact_id}-${index}`}>{evidenceValue(evidence)}<small>{evidence.source_label}</small></li>)}</ul> : <p>No supporting value was recorded.</p>}</div>{item.missing_information.length > 0 && <div><strong>Needed to resolve</strong><ul>{item.missing_information.map((missing, index) => <li key={`${missing.fact}-${index}`}>{missing.detail || 'Additional recorded information is required.'}</li>)}</ul></div>}</div><details className="audit-details"><summary>How this result is reproduced</summary><p>It uses the saved patient snapshot and approved trial version shown above.</p></details></article>)}</section>
