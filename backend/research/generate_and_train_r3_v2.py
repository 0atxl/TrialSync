...
OUTPUT_DIR = Path("/kaggle/working/trialsync_v2_bundle")
...
pt_burden = np.clip(base_sev * 0.7 + np.random.uniform(-0.2, 0.3, size=N_PARTICIPANTS), 0.0, 1.0)
comorb = np.clip(np.random.poisson(lam=(ages / 25)), 0, 4)
tx_burden = np.random.choice([0, 1, 2, 3, 4], p=[0.1, 0.25, 0.35, 0.20, 0.10], size=N_PARTICIPANTS)
travel = np.random.choice([0, 1, 2, 3, 4], p=[0.15, 0.30, 0.30, 0.18, 0.07], size=N_PARTICIPANTS)
support = np.clip(
...
scheduled_doses = np.random.choice([8, 12, 16, 30], p=[0.25, 0.20, 0.15, 0.40], size=N_PARTICIPANTS)
...
df.to_parquet(OUTPUT_DIR / "landmark_day30_features.parquet", index=False)
...
schema = {"version": "r4-day30-features-v2", "features": [{"name": col} for col in ALL_FEATURES]}
