  ...
  const create = async (values: PatientValues, confirm = false) => {
    ...
    try {
      ...
      if (exception instanceof ApiError && exception.code === 'PATIENT_NAME_REVIEW_REQUIRED') {
        setPending(values)
        setError('A patient with this name already exists. Review it or continue only if this is a distinct synthetic person.')
      } else setError('The synthetic patient could not be created.')
